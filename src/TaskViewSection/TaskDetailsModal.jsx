...
export default function TaskDetailsModal({ task }) {
  ...
  function ModalDetails({ name, icon, deadline, category }) {
    return (
      <div className="p-6 ">
        <div className="flex flex-col content-start gap-2">
          ...
          <h2 className="self-center p-0 m-0 font-extralight">{category}</h2>
          ...
          {deadline !== "Not specified" && (
            <div className="bg-transparent stats ">
              <div className="stat">{deadlineInfo(task.deadline)}</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        <label htmlFor={uniqueID} className="cursor-pointer modal">
          ...
          >
            ...
            />
            <ModalBackground color={colorStyleBlobHandler(task.colorStyle)} />
          </label>
        </label>
      </div>,
      portal
