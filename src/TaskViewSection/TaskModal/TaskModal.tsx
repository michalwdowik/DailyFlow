...
function ModalDetails({ task }: { task: TaskType }) {
...
function TaskName({ task }: { task: TaskType }) {
    return (
...
function TaskCategory({ task }: { task: TaskType }) {
    return (
        <h2 className="self-center p-0 m-0 font-extralight">{task.category}</h2>
    )
}

function Stars({ task }: { task: TaskType }) {
    return (
...
function ModalBackground({ color }: { color: string }) {
    return (
        <svg
