...
export default function Task({task, onChange, index, searchInput}) {
  ...
  return (
    <div className="">
      ...
      {(selectedTabCategory === "all" && task.name.toLowerCase().includes(searchInput.toLowerCase())) && (
          <label>
            <li className="py-2 border-0 border-b border-solid border-slate-200 sm:py-4">
              <div className="flex items-center space-x-4 ">
                <input
                  className={`${colorStyleCheckboxHandler(task.colorStyle)} checkbox `}
                  id={index}
                  type="checkbox"
                  onChange={onChange}
                  checked={task.done}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate transition text-slate-700 transition-delay-50 hover:text-gray-400">
                    {task.name}
                  </p>
      ...
      {(task.category === selectedTabCategory && task.name.toLowerCase().includes(searchInput.toLowerCase())) && (
        <label>
          <li className="py-2 border-0 border-b border-solid border-slate-200 sm:py-4">
            <div className="flex items-center space-x-4 ">
              <input
